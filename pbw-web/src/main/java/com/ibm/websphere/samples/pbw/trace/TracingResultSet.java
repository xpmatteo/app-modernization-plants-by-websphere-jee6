// ABOUTME: ResultSet wrapper that captures query results for tracing.
// ABOUTME: Collects all rows and writes trace event when ResultSet is closed.
...
/**
 * ResultSet wrapper that captures all rows for tracing.
 * Results are logged when the ResultSet is closed or when all rows have been consumed.
 */
public class TracingResultSet implements ResultSet {
    ...
    public TracingResultSet(ResultSet delegate, String sql, List<Object> params) {
    ...
    private void captureCurrentRow() throws SQLException {
        ...
        for (int i = 1; i <= columnCount; i++) {
            ...
            Object value = getColumnValue(i, columnType);
    ...
    private Object getColumnValue(int columnIndex, int columnType) throws SQLException {
        ...
        if (value instanceof byte[]) {
            return value; // TraceWriter will convert to hash
        }
        ...
        if (value instanceof Clob) {
            ...
            return "<clob:truncated>";
        }

        return value;
    }

    private void writeTraceIfNeeded() {
        if (!traced && TraceContext.isActive()) {
            traced = true;
            TraceWriter.writeJdbcQuery(sql, params, results);
        }
    }
