// ABOUTME: Statement wrapper that traces SQL queries executed via plain Statement.
// ABOUTME: Captures SQL strings and results for non-prepared statements.
...
/**
 * Statement wrapper that traces SQL execution for plain (non-prepared) statements.
 */
public class TracingStatement implements Statement {

    protected final Statement delegate;

    public TracingStatement(Statement delegate) {
    ...
    public ResultSet executeQuery(String sql) throws SQLException {
        ...
        if (TraceContext.isActive()) {
            TracingResultSet tracingRs = new TracingResultSet(rs, sql, new ArrayList<>());
            return tracingRs;
        }
        return rs;
    }
    ...
    public int executeUpdate(String sql) throws SQLException {
        ...
        if (TraceContext.isActive()) {
            TraceWriter.writeJdbcUpdate(sql, new ArrayList<>(), rows);
        }
        return rows;
    }
    ...
    public boolean execute(String sql) throws SQLException {
        ...
        if (TraceContext.isActive()) {
            if (result) {
                // Has ResultSet - will be traced when getResultSet() is called
            } else {
                int updateCount = delegate.getUpdateCount();
                TraceWriter.writeJdbcUpdate(sql, new ArrayList<>(), updateCount);
            }
        }
        return result;
    }
